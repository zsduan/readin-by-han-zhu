...
Page({
    data: {
        ...
        is_show_multiple : false, //是否打开倍数选择
        is_screen : false, //视频是否全屏
        is_controls : false , //是否展示视频倍数
        controls_text : "倍数" , //倍数
    },
    ...
    playAudio: function () {
        ...
        this.viewingRecordSaveTime(); //设置播放时间
    ...
    timeupdate: function (e) {
        ...
        if (app.globalData.paly_time >= 100) {
            $browsing.browsing(app.globalData.paly_time, 1, 2, false, parseInt(e.detail.currentTime));
            app.globalData.paly_time = 0;
    ...
    // 视频自然播放到结束
    endVideo: function (e) {
        $browsing.browsing(app.globalData.paly_time, 1, 2, true, wx.getStorageSync("duration")); //创建记录
        let timer_paly = this.data.timer_paly;
        clearInterval(timer_paly);
        timer_paly = null;
        this.setData({
    ...
    goSet: function () {
        ...
        setTimeout(() => {
            wx.redirectTo({
                url: '../setting/setting?pageUrl=' + this.data.pageUrls + '!is_video#true' + '@show_pup',
            })
        }, 1000)
    },
    // 初次设置播放
    viewingRecordSaveTime: function () {
        this.data.$api.post({
            url: "api/article/viewingRecordSaveTime",
            data: {
    ...
    changeMultiple:function(e) {
        ...
        if(multipleList[index].active){
            return ;
        }
        multipleList.forEach(v =>{
            v.active = false
        })
        multipleList[index].active = true;
        if(!this.data.is_play_video_now){
            bgMusic.playbackRate = multipleList[index].multiple;
        }else{
            this.videoContext = wx.createVideoContext('myVideo');
            this.videoContext.playbackRate(multipleList[index].multiple)
        }
        this.setData({
            is_show_multiple : false,
            multipleList : multipleList,
            controls_text : multipleList[index].show_multiple
        })
    },
    // 打开选择
    openMultiple:function() {
        if(!bgMusic.src && !this.data.is_play_video_now){
            wx.showToast({
              title: '请播放后在切换倍数哟',
              icon : "none",
              mask : true
        ...
        this.setData({
            is_show_multiple : true
        })
        setTimeout(()=>{
            if(this.data.is_show_multiple){
                this.setData({
                    is_show_multiple : false
                })
            }
    ...
    screenchange:function(e) {
        this.setData({
            is_screen : e.detail.fullScreen
        })
    },
    // 是否显示视频进度条
    controlsShow:function(e) {
        this.setData({
            is_controls : e.detail.show
        })
    },
    // 回到默认状态
    defaultMultiple:function() {
        let multipleList = this.data.multipleList;
        multipleList.forEach(v =>{
            if(v.multiple == 1){
                v.active = true;
